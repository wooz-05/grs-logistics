  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {product.listPrice && (
          <span className="absolute top-3 right-3 rounded-full bg-white/95 px-2.5 py-1 font-bold text-[#2f8f59] text-[11px]">
            -{discountPct(product)}%
          </span>
        )}
      </div>
      <div className="flex flex-1 flex-col px-5 pt-5">
        ...
        <h3 className="mt-1 font-bold text-[#242527] text-lg leading-snug transition-colors group-hover:text-[#EE1C4D]">
          {product.title}
        </h3>
        ...
        <div className="mt-3 flex items-baseline gap-2">
          <span className="font-bold text-[#242527] text-xl">{money(product.price)}</span>
          {product.listPrice && (
            <span className="text-neutral-400 text-sm line-through">{money(product.listPrice)}</span>
          )}
        </div>
      </div>
  ...
  return (
    <header className="sticky top-0 z-40 border-neutral-100 border-b bg-white/90 backdrop-blur">
      <div className="mx-auto flex w-[min(1180px,calc(100%-2rem))] items-center gap-4 py-3">
        <Link href="/ecommerce" aria-label="GRS store home" className="flex shrink-0 items-center gap-2">
          <Logo className="h-7 w-auto text-[#242527]" />
          <span className="hidden font-semibold text-[#242527] text-sm sm:inline">Store</span>
        </Link>
        <Link
          href="/"
          className="ml-auto hidden whitespace-nowrap font-medium text-neutral-500 text-sm transition-colors hover:text-[#242527] md:block"
        >
          Back to site
        </Link>
        <button
